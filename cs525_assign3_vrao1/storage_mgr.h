...
//Changed from original version of the code; made it's storage type extern

typedef struct _allFiles allFiles;
extern allFiles *arrOfFiles; // Doubly Link List head pointer
...
extern RC readBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage, int isItEmpty);
...
extern RC readDiskIntoStream(int fHandle ,SM_PageHandle memPage, int bufLen);
extern RC readStreamThenWrite(int fHandle, SM_PageHandle memPage,int pageLen); 
#endif
